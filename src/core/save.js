...
const wingGrowthDef=()=>({firepower:0,health:0,rate:0,intercept:0});
const RUN_DEF=()=>({saveVersion:SAVE_VERSION,lvl:0,score:0,kills:0,time:0,cycle:0,pts:0,
  up:{hp:0,spd:0,atk:0,def:0,cdr:0},eq:{armor:0,track:0,fire:0,comp:0},
  wingmanGrowth:wingGrowthDef(),hull:'balanced',wing:'flex'});
let RUN=RUN_DEF(),lvlSnap=null;
function saveRun(){ if(ST&&ST.debugActive)return;   /* §27: Debug 会话不写正式存档 */
  try{RUN.saveVersion=SAVE_VERSION;localStorage.setItem('trSave',JSON.stringify(RUN));}catch(e){} }
function migrateRun(raw){
  const r=raw&&typeof raw==='object'?raw:{};
  const out=Object.assign(RUN_DEF(),r);
  out.up=Object.assign({hp:0,spd:0,atk:0,def:0,cdr:0},r.up||{});
  out.eq=Object.assign({armor:0,track:0,fire:0,comp:0},r.eq||{});
  out.wingmanGrowth=Object.assign(wingGrowthDef(),r.wingmanGrowth||r.up&&r.up.wingmanGrowth||{});
  out.saveVersion=SAVE_VERSION;
  return out;
}
function loadRun(){try{const s=localStorage.getItem('trSave');
  if(s){RUN=migrateRun(JSON.parse(s));return true;}}catch(e){}return false;}
...
function refundAll(){
  const g=RUN.wingmanGrowth||wingGrowthDef();
  RUN.pts+=RUN.up.hp+RUN.up.spd+RUN.up.atk+RUN.up.def+(RUN.up.cdr||0)+g.firepower+g.health+g.rate+g.intercept;
  RUN.up={hp:0,spd:0,atk:0,def:0,cdr:0}; RUN.wingmanGrowth=wingGrowthDef();
}
