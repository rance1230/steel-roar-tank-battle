...
function spawnEnemyAt(kind,boss,x,y){
  ...
  const base={id:(ST.nextEnemyId=(ST.nextEnemyId||0)+1),kind,boss:!!boss,x:x,y:y,ox:x,oy:y,a:0,
    stun:0,flash:0,ramCd:0,touchCd:0,dist:0,jitter:0,flying:null,kvx:0,kvy:0,
    mass:boss?'fortress':(isTank?'medium':'light')};
  if(isTank) enemies.push(Object.assign(base,{r:9,
    hp:44*cfg.hp*dm.hp*cycHp(),maxHp:44*cfg.hp*dm.hp*cycHp(),speed:44,   /* v1.8 W6平衡: 26→44 */
    prefMin:120,prefMax:200,range:250,fireT:rnd(1,2.4),fireCd:2.6/cfg.rate/dm.react*(RUN.lvl>=4?0.9:1),   /* v1.8 W6平衡: L4+ 射速+10% */
    ...
    hp:26*cfg.hp*dm.hp*cycHp(),maxHp:26*cfg.hp*dm.hp*cycHp(),speed:78,   /* v1.8 W6平衡: 17→26 */
...
function fireMG(){ shotsFired++; const a=player.ta+rnd(-0.05,0.05);   /* v1.8 W2: 沿炮塔; raw 3, atk 由§3管线乘 */
  ...
  shot(mx,my,a,430,2.2,true,'mg');   /* v1.8 W6平衡: 3→2.2 */
...
function fireCannon(){ const a=player.ta+rnd(-0.02,0.02);   /* v1.8 W2: 沿炮塔; raw 24, atk 由§3管线乘 */
  ...
  shot(mx,my,a,320,18,true,'shell',{cause:'shot'});   /* v1.8 W6平衡: 24→18 */
...
function updPlayer(dt){
  ...
  /* ---- v1.8 W3: 移动/漂移 5参数模型 (契约§2) ----
     左杆=desired velocity → 世界系 accel 逼近; bodyA 以 turnRate 逼近速度方向;
     速度投影车体系 forward/lateral, lateral 以 lateralGrip 衰减 → 车身跟不上即侧滑甩尾;
     制动按速度插值(松键不急停, 满速松0.3s仍>30%速度);
     地形: 冰 grip×0.3 / 河 grip×0.6 / 熔岩 grip×1(仅减速) / 泥 speed×0.55。 */
  ...
  const mv=hullCfg().move;
  const tid0=tileAtPx(p.x,p.y);
  const mul=slowMul(tid0);
  const ice=tid0===3&&RUN.lvl===4, river=tid0===3&&!ice&&RUN.lvl!==6;
  const gripMul=ice?0.3:river?0.6:1;
  let sprint=false;
  ...
  const maxSpd=p.speed*mul*(sprint?1.9*mv.sprint:1)*(COMBO.od?1.1:1);
  const spd0=Math.hypot(p.vx,p.vy);
  const ACCEL=maxSpd/0.16*mv.accel;
  const FRICT=maxSpd/0.55*mv.brake*(0.3+0.7*Math.min(1,spd0/maxSpd));   /* 低速尾段更缓, 不硬锁零 */
  const tvx=ix*maxSpd, tvy=iy*maxSpd;
  const dv=(im>0.01?ACCEL:FRICT)*dt;
  p.vx+=clamp(tvx-p.vx,-dv,dv);
  p.vy+=clamp(tvy-p.vy,-dv,dv);
  /* 车体系分解: 侧向分量按 grip 指数衰减 (grip低→侧滑持续→甩尾) */
  const ca=Math.cos(p.bodyA), sa=Math.sin(p.bodyA);
  let fV=p.vx*ca+p.vy*sa, lV=-p.vx*sa+p.vy*ca;
  lV*=Math.exp(-mv.lateralGrip*gripMul*7*dt);
  p.vx=fV*ca-lV*sa; p.vy=fV*sa+lV*ca;
  p.slip=Math.abs(lV);
  const spd=Math.hypot(p.vx,p.vy);
  p.moving=spd>15;
  const px0=p.x, py0=p.y;   /* v1.7: 彗尾按实际位移增长(顶墙卡住时位移≈0, 尾巴收缩) */
  /* v1.8 W3.5: 碰撞移动走 moveCircEx — 撞墙按法向冲击分级反馈(速度反弹, 斜擦vn≈0不误判) */
  const mc=moveCircEx(p,p.vx*dt,p.vy*dt,p.r,p.vx,p.vy);
  if(mc.hit){
    const vn=p.vx*mc.nx+p.vy*mc.ny;
    if(vn<0){
      const impact=-vn;
      p.vx-=mc.nx*vn*1.25; p.vy-=mc.ny*vn*1.25;   /* 去除法向分量+25%轻反弹 */
      if(impact>260){ kickTier(2,Math.atan2(mc.ny,mc.nx));
        burst(p.x+mc.nx*p.r,p.y+mc.ny*p.r,10,[PAL.gold,PAL.lite,PAL.white],85,0.3); SFX.ram(p.x,p.y); }
      else if(impact>120){ kickTier(1,Math.atan2(mc.ny,mc.nx));
        burst(p.x+mc.nx*p.r,p.y+mc.ny*p.r,4,[PAL.lite,PAL.white],55,0.22); }
    }
  }
  ...
  if(dist2(e.x,e.y,pl.x,pl.y)<(e.r+pl.r+3)*(e.r+pl.r+3)){
    if(e.ramCd<=0){
      ...
      if(sp2>120&&facing&&!pl.breach&&!e.flying){ enterBreach(e); }
        ...
        /* v1.8 W6平衡: 冲撞=高风险高回报 — 非BOSS 暴击55×atk 即杀 "CRUSH!";
           BOSS 60×atk 大伤害+stagger, 永不即杀(保底1hp) */
        if(e.boss){
          applyDamage(e,60,'ram');
          if(e.dead||e.hp<=0){ e.dead=false; e.hp=Math.max(1,e.hp); }   /* BOSS 永不即杀 */
        ...
        e.kvx=(e.kvx||0)-nx2/nl*kbV; e.kvy=(e.kvy||0)-ny2/nl*kbV;
        const mE=e.boss?6:(e.kind==='tank'?1.3:0.9), mP=hullCfg().move.mass;
        const rec=kbV*mE/(mE+mP)*0.5;
        pl.vx+=nx2/nl*rec; pl.vy+=ny2/nl*rec;
        burst((e.x+pl.x)/2,(e.y+pl.y)/2,e.boss?16:14,[PAL.gold,PAL.white,PAL.lite],e.boss?110:95,0.4);
        RUN.score+=25;
        SFX.ram((e.x+pl.x)/2,(e.y+pl.y)/2);
        kickTier(e.boss?2:3,Math.atan2(e.y-pl.y,e.x-pl.x));   /* 非BOSS碾杀→强档; v1.7冲撞分级 */
      } else if(e.touchCd<=0&&pl.inv<=0){
        e.touchCd=1.2; damagePlayer(6,Math.atan2(ny2,nx2),true);
      }
      /* v1.8 W3.5: 玩家↔敌法向冲量按质量比交换(闭合才作用, 分离即停; 只改速度) */
      {
        const rvn=((pl.vx-(e.kvx||0))*nx2+(pl.vy-(e.kvy||0))*ny2)/nl;
        if(rvn<0){
          const mE2=e.boss?6:(e.kind==='tank'?1.3:0.9), mP2=hullCfg().move.mass;
          const J=-1.1*rvn/(1/mP2+1/mE2);
          pl.vx+=nx2/nl*J/mP2; pl.vy+=ny2/nl*J/mP2;
          e.kvx=(e.kvx||0)-nx2/nl*J/mE2; e.kvy=(e.kvy||0)-ny2/nl*J/mE2;
        }
      }
    }
  }
...
function updEnemies(dt){
  ...
  for(const e of enemies){
    ...
    if(e.stun>0){e.stun-=dt; if(e.boss&&e.chgPhase){e.chgPhase=null;e.chg=rnd(4,6);}
      /* v1.8 W3.5: 眩晕也受击退速度场作用(被撞飞的感觉), 经碰撞路径 */
      if(e.kvx||e.kvy){ moveCirc(e,e.kvx*dt,e.kvy*dt,e.r);
        e.kvx*=Math.exp(-6*dt); e.kvy*=Math.exp(-6*dt); }
      continue;}
    ...
    if(e.kind==='tank'&&!e.boss&&!e.flying&&e.stun<=0){
      if(e.dodgeT>0){
        e.dodgeT-=dt;
        if(e.dodgeT<=0){
          if(Math.random()<DODGE_P[SET.diff]){
            e.kvx=(e.kvx||0)+Math.cos(e.dodgeA)*150; e.kvy=(e.kvy||0)+Math.sin(e.dodgeA)*150;
            e.telegraph=0;
          }
        }
      } else {
        ...
        if(e.dodgeCd<=0){
          const th=shots.find(s=>s.friendly&&!s.refl&&s.kind!=='mg'&&
            dist2(s.x,s.y,e.x,e.y)<110*110&&((e.x-s.x)*s.vx+(e.y-s.y)*s.vy)>0);
          if(th){ e.dodgeCd=rnd(1.5,3); e.dodgeT=0.12+rnd(0,0.18); e.telegraph=0.24;
            e.dodgeA=Math.atan2(e.y-th.y,e.x-th.x)+Math.PI/2*(e.orb||1); }
        }
      }
