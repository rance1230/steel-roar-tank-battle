...
**当前版本 v1.9.0「日系机甲换装」**（Release 线：v1.8.1 UI 全面换装 → **v1.9.0 日系机甲三机体 · 机枪自动瞄准+锁定 · 空袭护航投弹 · 僚机成长 · NG+ 重编队**）

> 架构里程碑线（vNext-v3.x）：v14 高清 AI 立绘 → v15 俯视 16 向单位 + 地形图集 → v1.5 七主题关卡与玻璃 UI → v1.6 机动特效与 BOSS 护卫队 → v1.7 手感三改（震动分级 / 冲刺残影 / 3D 等离子护罩）→ v1.8.1 UI 资产换装 → v1.9.0 日系机甲
...
- **v1.9.0 日系机甲三机体**：原创机甲风底盘+独立炮塔全面替换——底盘 16 方向图集、炮塔 360° 连续旋转；旧素材逐级回退。
- **v1.9.0 机枪自动瞄准 + 锁定**：机枪默认自动咬住 360px 内最近敌；按 I（触屏◎/手柄）锁定当前目标、再按按距离循环切换；右摇杆随时接管手动瞄准。
- **v1.9.0 空袭护航投弹**：支援机绕己方坦克盘旋，向锁定/最近敌精准投弹；无目标时持续盘旋不浪费。
- **v1.9.0 僚机成长**：僚机火力/生命/射速/拦截四维培养，与机体共用点数、可全额退回；旧存档自动迁移（v3）。
- **v1.9.0 NG+ 重编队**：通关后重选机体与僚机、点数继承进入下一周目。
